...
function out = acquireImage(arg)
    if isstring(arg) && arg == "init"
        cams = webcamlist;
        cam = webcam(cams{1}); % for a device with an integrated cam the webcam is #2. change if needed
        
        pause(.2) %Maybe needed if camera needs time to initialize to take
        % a good picture
        
        cam.Resolution = '640x480';
        cam.Saturation = 128; %128 makes the colors pop more and hopefully makes detection a little easier, default 100
