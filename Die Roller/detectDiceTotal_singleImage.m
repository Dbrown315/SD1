...
% Dark pip settings  %with no light 0.25 and 20
addParameter(p, "DarkPixelThresh", 0.35, @(x)isnumeric(x) && isscalar(x));%og = 0.32
addParameter(p, "MinPipArea", 20, @(x)isnumeric(x) && isscalar(x)); %og = 20
addParameter(p, "MaxPipArea", 500, @(x)isnumeric(x) && isscalar(x));
%works with light one level from the highest level at o.32 and 20
% Shape filtering
addParameter(p, "MaxEccentricity", 0.92, @(x)isnumeric(x) && isscalar(x));
